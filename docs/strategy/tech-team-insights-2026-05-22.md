...
**Contexto:** Sessão Eco Adventure (Lívia Tagliari, Gabriel Bonfim, Victor). Foco: arquitetura SDR/atendimento WhatsApp. Aplicáveis ao Studio Tirra.
...
5. **Modelo sistemático + criatividade baixa.** Gabriel: "flexibilidade pelo texto = IA faz o que quer". Travar pela config do modelo, dar tom-de-voz pelo prompt.
...
8. **Prompt muito grande = IA pula instruções.** Gabriel: "IA lê como humano com preguiça — introdução, meio, fim, pula o miolo". Refatorar prompt monolito ("Megazord") em prompts menores especializados por agente.
...
- **P1:** Agente de transcrição de áudio (Gabriel: "todo mundo manda áudio no WhatsApp").
