...
```
(A mensagem padrão de passagem pra recepção da própria I.6 continua valendo.)
...
## 7. 18/08/2026 — expediente, anti-duplicata, vocabulário (Victor cola)

Cópia canônica atualizada em `docs/prompts/tess-conversa-v3-clean.md`. Colar **depois** do deploy desta fatia de backend (idempotência + guarda de expediente). Sem o backend, o modelo ainda pode emitir tag duplicada e o POST passa.

### ROLE — SUBSTITUIR a frase "salão premium"

```
Você é a assistente virtual do Studio Tirra, salão de referência em São Caetano do Sul/SP. Supervisor humano: a recepção.
```

### I.6 — ADICIONAR bullet (antes do combo sequencial)
...
### I.8 — ADICIONAR no bullet "NÃO finja que o salão está aberto"

```
O backend também não envia "Te esperamos" quando o salão está fechado agora.
```

### I.11 e I.12 — INSERIR depois de I.10 (antes de # S — STYLE)

Copiar as seções **I.11 — Grade e expediente** e **I.12 — Depois de criar, não crie de novo** de `docs/prompts/tess-conversa-v3-clean.md`.

### VOCABULÁRIO + FORMATAÇÃO WHATSAPP — INSERIR depois de # S — STYLE (antes de # P — POLICIES)

Copiar os blocos **VOCABULÁRIO AO CLIENTE** e **FORMATAÇÃO WHATSAPP** do mesmo arquivo.

### VALIDE ANTES DE ENVIAR — COLAR no fim do prompt
...
## 8. 18/08/2026 tarde — v3.1.1 DADOS_CLIENTE (Victor cola o arquivo INTEIRO)

Substituir o prompt do TESS 46589 pelo conteúdo completo de `docs/prompts/tess-conversa-v3-clean.md` (carimbo `v3.1.1`). Não é patch incremental: o I.12 da v3.1.0 pedia cadastro completo e precisa sair.

---

## Rastreabilidade (sem diff — registrar versão aplicada)
Ao aplicar, anota no MEMORY/story: **data + versão (`v3.1.1`)**. O prompt vivo é `docs/prompts/tess-conversa-v3-clean.md`; snapshots em `docs/prompts/archive/`; changelog em `docs/prompts/CHANGELOG-46589.md`.
