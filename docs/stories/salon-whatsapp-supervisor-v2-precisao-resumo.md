...
**Status:** InProgress — ✅ **Camada 1 re-validada GO pelo @po (9/10, 2026-05-29) e rota de fetch escolhida pelo @architect (Rota A — pull Kapso no digest). Fonte do AC2 = API Kapso (`kapso.direction`, confirmada em prod). Liberada para re-dev da Camada 1.** Camada 2 aguarda prompt 46590 (Fase 0).
...
- [ ] **AC11:** **Confiabilidade + prova-de-fonte (fecha o gap que deixou o no-op passar):** dryRun (`POST /admin/trigger-supervisor?dryRun=1` → `GET /admin/last-digest`) **conclui sem travar** E o log de funil (AC4) mostra **`filtradas > 0` em dados REAIS de prod** (i.e. o filtro Kapso efetivamente removeu conversas — não pode repetir o `127 → filtradas 0`). ⚠️ **Testes unitários com Kapso mockado NÃO satisfazem este AC** — eles passaram 44/44 no ciclo anterior e mesmo assim o filtro era morto em prod; só o dry-run em dados reais comprova que a fonte carrega o sinal. Este AC só é marcado [x] após smoke em produção.
