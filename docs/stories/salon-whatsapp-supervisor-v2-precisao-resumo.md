...
- **Feedback real da recepção** (transcrição de áudio, 2026-05-29) — inline em Contexto

## Contexto

O **supervisor matinal** (`backend/supervisor.js`, agente TESS 46590) roda 7h (ter-sáb), classifica as conversas das últimas ~24h (62h na terça) **uma a uma via TESS**, ranqueia e manda um resumo no WhatsApp pro Tiago + recepção (destinatários extras adicionados nesta sessão via `SUPERVISOR_EXTRA_PHONES`).

**Feedback real da recepção (supervisor de turno) sobre o resumo que recebeu:**
...
2. **Cliente sumiu** — conversa "aberta" porque o cliente parou de responder. É o backlog de follow-up da recepção, que ele trata do jeito dele (re-pinga depois) — não é urgência.

**Intent do produto (Victor, decisão-chave):** o resumo **NÃO substitui** o método da recepção (ordem de chegada / FIFO). Ele **continua** com o método dele. O resumo é um **upgrade híbrido**: deve **puxar pra frente da fila** as poucas mensagens que genuinamente merecem furar o FIFO (urgência/importância real). A recepção atende essas primeiro e **depois segue o FIFO normal**.
...
1. **Quem falou por último** — se o **salão** (bot OU recepção/recepção respondendo manual) falou por último → já tratado ou é vez do cliente (silêncio ≠ urgência). Se o **cliente** falou por último → está **aguardando resposta** = acionável. O agente tem as mensagens mas não pesa isso.
...
- Filtro duro: considera só conversas onde o **cliente falou por último** (aguardando salão), determinado pela **fonte Kapso** (`kapso.direction`) — NÃO mais por `conversation_history` (que é cega pro salão). Isso sozinho cobre os 3 baldes de falso-positivo da recepção.
...
- Prompt reescrito pro critério "fura-fila" + saída no formato híbrido (respeita o FIFO da recepção).
...
- [x] **AC2 (reescrito — fonte Kapso, não `conversation_history`):** Em `runMorningTriage`, antes de classificar, manter **só conversas onde o cliente falou por último**, determinado pela **fonte de last-speaker da Kapso** (`kapso.direction`):
  ...
  - **Resultado (testável):** **excluir** toda conversa cuja **última mensagem é `outbound`** (já respondida pelo salão / vez do cliente); **manter** só as com última mensagem `inbound` (cliente aguardando). Os 8 falsos-positivos do feedback da recepção **devem cair fora**; as 2 boas **devem permanecer**.
...
- [x] **AC6 (código):** Enriquecer o input de `classifyConversation` com: `QUEM_FALOU_POR_ULTIMO` (`cliente`|`salao`, derivado da fonte Kapso do AC2 — reusar, não recomputar), `TEM_AGENDAMENTO` (bool + status/quando se houver), `HORARIO_ULTIMA_MSG_CLIENTE` (pra recepção encaixar no FIFO).
...
- [x] **AC8 (código):** Saída reformulada pro **híbrido** (`renderDigest`): a mensagem deixa claro *"estes furam sua fila — atenda primeiro, depois siga sua ordem de chegada normal"*. Cada item: motivo curto + **horário de chegada** + ação sugerida. Curto e escaneável (tempo da recepção é precioso). Vazio → mensagem positiva ("nada fura a fila hoje, bom dia").

### Validação
- [ ] **AC9:** **Precisão (2 lados):** validar contra casos reais via dryRun + revisão manual:
  - **Falso-positivo:** as conversas sinalizadas são de fato "cliente aguardando + acionável" (idealmente reproduzir os casos da recepção: sinalizar as 2 boas, não as 8).
  - **Falso-negativo (guardrail):** revisar uma amostra das conversas **excluídas pelo filtro** e confirmar que nenhuma era urgência real. Edge case aceito p/ MVP: bot respondeu sem resolver (salão falou por último) → fica fora do destaque, mas o **FIFO da recepção pega** (alinhado ao que ele pediu: menos ruído).
...
- [~] **Camada 2 — input + prompt (AC6-AC8)** — código DONE; prompt 46590 sendo colado por Victor; aguarda deploy+smoke
  - [x] Enriquecer input do `classifyConversation` (AC6): `QUEM_FALOU_POR_ULTIMO` (reusa Map Kapso), `TEM_AGENDAMENTO` (sinal via `fetchFutureBookings`, fail-open), `HORARIO_ULTIMA_MSG_CLIENTE` (`lastClientMsgTs`)
  - [x] `renderDigest` híbrido "fura-fila" (AC8): enquadramento "atenda primeiro, depois siga o FIFO" + horário de chegada (`formatArrival`) + marcador de agendamento + vazio→msg positiva
  - [ ] Reescrever prompt 46590 (no TESS) — **Victor colando** (prompt fornecido pelo time; rework do real, binário escalate/ignore, mata silêncio/resolvido)
  - [ ] Deploy do par casado (código + prompt) + smoke do dryRun
- [ ] **Validação (AC9-AC11)** — dryRun, precisão vs casos da recepção, custo, confiabilidade
...
- Categorias já existentes no scoring (:114): `reclamacao, pedido_humano, conflito_agenda, qualidade_ruim, silencio_meio_conversa, falso_positivo`. Note que `silencio_meio_conversa` é exatamente o balde "cliente sumiu" que a recepção quer **fora** do destaque.

### ⚠️ `conversation_history` é CEGO pro salão (por que o AC2 v1 falhou)
- A tabela só registra o **bot** (`role='user'`=cliente, `role='assistant'`=bot, `agent='passive'`/`'human'`). As **respostas manuais da recepção/recepção pelo WhatsApp NÃO entram aqui** — o webhook recebe esses outbound (`server.js:1295`) mas os descarta.
...
`SUPERVISOR_EXTRA_PHONES` (CSV de destinatários extras do resumo) já está no `supervisor.js` (:22, :191) e em prod (recepção `5518998240447` configurado). Esta story **não mexe** em destinatários — só na **seleção/qualidade** do conteúdo.

### O que NÃO fazer
- Não substituir o FIFO da recepção — **complementar** (puxar pra frente só o que fura a fila).
...
- **Integração/smoke:** dryRun real no VPS (AC9-AC11) — conclui, `filtradas > 0`, precisão vs casos da recepção, log de custo.

## Change Log

| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 2026-05-29 | 2.2 | **@dev: nome do cliente no digest (UX, pedido do Victor).** O digest mostrava só telefone; agora mostra `Nome (telefone)`. Nome resolvido com fallback `clients.name` → **`contact_name` da Kapso** (capturado no mesmo pull do filtro — cobre clientes NOVOS que não estão na tabela `clients`, justo os "oportunidade_quente") → telefone. `buildLastSpeakerMap` agora também retorna `contactNames`; `classifyConversation` usa o nome resolvido em `DADOS_CLIENTE.nome` (ajuda o agente também). Sem novo I/O (reusa o pull Kapso + o `fetchClientInfo` que já existiam). **73/73 testes** (+2). Branch `feature/supervisor-v2-digest-nome`. Próximo: deploy + dryRun pra ver com nomes. | @dev Dex |
| 2026-05-29 | 2.1 | **@devops: ✅ SMOKE Camada 2 VERDE em prod (par código+prompt).** Deploy da branch `feature/supervisor-v2-camada2` no VPS (sem merge). Prompt novo já ativo no 46590 (Victor colou). DryRun: funil `total=127 → filtro 38 (matched 127/127, excluded_outbound=89), window_fully_covered=true, page_cap_hit=false, 0 erros, 143s`; **sinalizadas 8** (vs 10 com prompt velho). **Digest com qualidade fura-fila real:** os itens são novo_agendamento (indicação querendo 3 áreas hoje), oportunidade_quente (leads novos prontos pra fechar), tempo_sensivel (agendar hoje), conflito_agenda (remarcar) — sumiram "cliente sumiu"/"já respondida". Enquadramento híbrido + horário de chegada presentes. AC6/AC7/AC8 validados; AC9 passa qualitativamente (itens sinalizados são acionáveis). Próximo: merge em main + restaurar VPS→main (aguarda OK do Victor). | @devops Gage |
| 2026-05-29 | 2.0 | **@dev: Camada 2 — CÓDIGO concluído (AC6+AC3+AC8).** Com o prompt REAL do 46590 em mãos (Victor colou no chat; `get_agent` só dá metadados, confirmado), foi rework (não reescrita cega): reorientado de "qualidade do bot→Tiago" pra "fura-fila pra recepção", `decision` binário escalate/ignore, `silencio_meio_conversa`/resolvido→ignore (mata os 2 baldes da recepção), ramos mortos sync_tag/sync_heuristic removidos. **Confirmado que 46590 só é chamado pelo cron** (`supervisor.js`) — os HANDOFF_HUMAN no server.js são do bot 46589 → trocar o prompt é seguro. **Código:** `classifyConversation` enriquecido (AC6: `QUEM_FALOU_POR_ULTIMO` reusa Map Kapso, `TEM_AGENDAMENTO` via `fetchFutureBookings` fail-open, `HORARIO_ULTIMA_MSG_CLIENTE`), `renderDigest` híbrido (AC8: "fura a fila / depois FIFO" + horário de chegada + marcador booking), booking como SINAL (AC3, não filtro). Contrato JSON de saída preservado. **71/71 testes** (+9 Camada 2). Branch `feature/supervisor-v2-camada2`. Próximo: deploy do par casado (código + prompt) + smoke. AC7 = Victor colando no TESS. | @dev Dex |
...
**Camada 2 — CÓDIGO CONCLUÍDO 🟢 (AC6 input enriquecido + AC3 sinal booking + AC8 render híbrido)** (`backend/supervisor.js`, branch `feature/supervisor-v2-camada2`)
- **AC6 (`classifyConversation`):** input ganhou `QUEM_FALOU_POR_ULTIMO` (`'cliente'|'salao'|null`, derivado do Map Kapso do AC2 — **reusado, não recomputado**), `TEM_AGENDAMENTO` (`{tem,quando,status}` ou `{tem:false}`), `HORARIO_ULTIMA_MSG_CLIENTE`.
- **AC3 (sinal, não filtro):** `fetchFutureBookings()` — 1 query de agendamentos futuros ativos (`scheduled_at >= NOW()` e `status NOT IN (cancelled,no_show)`), Map por telefone normalizado (mais próximo por telefone), **fail-open** (erro de DB → Map vazio, nunca derruba o digest). Booking NÃO exclui conversa — vai como sinal pro agente.
- **AC8 (`renderDigest`):** reescrito pro híbrido — "X conversas que furam a fila — atenda primeiro, depois siga sua ordem de chegada normal"; cada item com **horário de chegada** (`formatArrival`, fuso do salão), marcador `📅 já tem agendamento`, motivo + ação; vazio → "Nada fura a fila hoje". Rodapé reforça o FIFO.
- **AC7 (prompt 46590):** rework fornecido (reorienta "qualidade do bot→Tiago" → "fura-fila pra recepção"; `decision` binário escalate/ignore; `silencio_meio_conversa`/resolvido → ignore/score 0; novos sinais; remove ramos mortos sync_tag/sync_heuristic). **Victor colando no TESS.** Contrato de saída JSON preservado (scoreFromVerdict/renderDigest intactos).
- **Helpers novos:** `fetchFutureBookings`, `lastClientMsgTs`, `formatArrival`. `runMorningTriage` deriva os 3 sinais por conversa (reusa `lastSpeaker.map` + `bookingMap`) e carrega `last_client_ts`/`tem_agendamento` no item ranqueado pro render.
- **Testes:** +9 testes Camada 2 (lastClientMsgTs, formatArrival, fetchFutureBookings incl. fail-open, classifyConversation injeta os 3 sinais, renderDigest híbrido) → suíte backend **71/71**.
- **Pendente:** deploy do par casado (código + prompt colado) + smoke do dryRun (conferir digest mais enxuto/fura-fila, sem erro). AC9 (precisão fina) avaliada no smoke + revisão manual vs casos da recepção.

### File List
- `backend/supervisor.js` (modificado) — RE-DEV Camada 1 (Kapso last-speaker) + Camada 2 (`classifyConversation` enriquecido, `fetchFutureBookings`, `lastClientMsgTs`, `formatArrival`, `renderDigest` híbrido; `runMorningTriage` deriva sinais por conversa); exports atualizados
- `backend/test/supervisor.test.js` (modificado) — Camada 1 (shape real Kapso) + Camada 2 (+9 testes) → 71 total
- `infra/docker-compose.yml` (modificado, Camada 1) — `KAPSO_API_BASE_URL` + `KAPSO_PHONE_NUMBER_ID` no backend
