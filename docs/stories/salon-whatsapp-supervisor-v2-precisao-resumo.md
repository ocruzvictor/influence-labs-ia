...
- [x] **AC6 (código):** Enriquecer o input de `classifyConversation` com: `QUEM_FALOU_POR_ULTIMO` (`cliente`|`salao`, derivado da fonte Kapso do AC2 — reusar, não recomputar), `TEM_AGENDAMENTO` (bool + status/quando se houver), `HORARIO_ULTIMA_MSG_CLIENTE` (pro Gabriel encaixar no FIFO).
...
- **AC7 (prompt 46590):** rework fornecido (reorienta "qualidade do bot→Tiago" → "fura-fila pro Gabriel"; `decision` binário escalate/ignore; `silencio_meio_conversa`/resolvido → ignore/score 0; novos sinais; remove ramos mortos sync_tag/sync_heuristic). **Victor colando no TESS.** Contrato de saída JSON preservado (scoreFromVerdict/renderDigest intactos).
...
- **Pendente:** deploy do par casado (código + prompt colado) + smoke do dryRun (conferir digest mais enxuto/fura-fila, sem erro). AC9 (precisão fina) avaliada no smoke + revisão manual vs casos do Gabriel.
