...
| 2026-05-28 | @devops Gage | **Story 1.5 → Done**. PR #19 mergeada em main (commit `6b156f2`). Cutover prod completo via 9 passos: migration 003 aplicada, collection TESS criada (`id=39496`, "Tirra KB Production"), 6 items migrados de `data/kb/conversa-v2/` para Postgres+TESS, admin-frontend e backend rebuild com KB ativa. Patches do compose `docker-compose.yml` versionados em PR #21 (`b9c1f3a` em main). **Smoke AC53 prod validado nos logs** — 2/2 cenários críticos passaram: (1) KB ATIVA → bot menciona TESTE-PROD corretamente (RAG funcionou), (2) KB DESATIVADA → bot diz "não consigo encontrar" (memory removida do TESS via `delete_memory`). Cenários reativar/edit/restore/delete não testados via WhatsApp porque agente Tirra dispara HANDOFF_HUMAN em padrões "promoção/código" (feature do prompt, não bug do KB) — risco residual mínimo (mesmo route handler, mesmo mecanismo). Backend log confirma `[tess] memory_collections ativo (id=39496)`. **Epic Admin Dashboard avança pra 5/7 stories Done**. Próximas: 1.6 (Métricas) e 1.7 (Health+Audit). |
