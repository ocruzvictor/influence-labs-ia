...
REQUEST-RESOLUTION: |
  ...
  - "verifique migalhas de contexto" → *fingerprint-check → carrega checklists/fingerprint-checklist.md
  - "audite tarefas burras" → *task-d-audit → carrega checklists/task-d-audit.md
...
command_loader:
  ...
  "*fingerprint-check":
    description: "Validar Migalhas de Pão — contexto no sweet spot para próximo executor"
    requires:
      - "checklists/fingerprint-checklist.md"
    optional:
      - "data/syncra-methodology.md"
      - "data/l0-intake.md"
    output_format: "Relatório PASS/FAIL por dimensão (formato, regras, tom, marca, densidade)"

  "*task-d-audit":
    description: "Auditar tarefas burras (Tipo D) — candidatas a Worker/Agente"
    requires:
      - "checklists/task-d-audit.md"
    optional:
      - "data/syncra-methodology.md"
    output_format: "Inventário D/H/Hy + top 5 extinções + horas recuperáveis"
...
metadata:
  ...
  upgraded: "2026-09-08"
  changelog:
    - "1.1.3: Anatomia oficial de 8 campos nas 15 tasks (aliases no first-block)"
    - "1.1.2: L0 intake obrigatório antes de *eng-map em projeto estrangeiro"
    - "1.1.1: Anatomia AIOX nas 15 tasks (status, execution_type, responsible_executor)"
    - "1.1: Syncra addendum — L0–L3, Migalhas, Tarefas D, *fingerprint-check, *task-d-audit"
...
heuristics:
  ...
  - "Syncra — Redução de Cliques: sucesso = menos interação humana em tarefas burras."
  - "Syncra — Migalhas de Pão: contexto no sweet spot (nem vazio, nem pão inteiro) ou a IA alucina."
  - "Syncra — Martelo: IA executa, humano assina. Accountability nunca delega à máquina."
  - "Syncra — Serial Killer de Entidades: entidade nasce, transita e morre — sem cemitério de backlog."
  - "Syncra — Herança L0→L3: DNA propagado de cima para baixo; não re-briefar o que já herdou."
  - "Syncra — Se não está no sistema (Journey Log), não aconteceu."
...
commands:
  ...
  - name: "fingerprint-check"
    visibility: [full]
    description: "Validar Migalhas de Pão (Syncra) — formato, regras, tom, marca"
    loader: "checklists/fingerprint-checklist.md"

  - name: "task-d-audit"
    visibility: [full]
    description: "Auditar tarefas burras Tipo D — extinção via Worker/Agente"
    loader: "checklists/task-d-audit.md"
