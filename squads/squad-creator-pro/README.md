...
## LOCAL STATUS (influence-labs-ia)

> Appended locally — does not modify upstream product claims above.

**This workspace:** `pro_mode: false`, `local_factory: true` (`config.yaml` v0.1.0-local).

| Shipped here | Not shipped (wait for official Pro body) |
|--------------|------------------------------------------|
| `config.yaml`, `workflows/wf-brownfield-upgrade-squad.yaml`, `README.local.md` | 34 Pro tasks, 3 specialist agents, mind cloning, axioma, model routing |
| Structural upgrade via free `../squad-creator/tasks/upgrade-squad.md` | Qualitative DNA extraction |
| Optional Pedro L4 via `../pedro-valerio-squad` | Paid Pro workflow library |

**Run:** `@squad-chief *upgrade-squad pedro-valerio-squad --mode=audit` — details in [README.local.md](./README.local.md).

**Protected (audit/plan only):** `tess-nightwatch`, `tess-floor-lexicon`, `lib-forge`.
